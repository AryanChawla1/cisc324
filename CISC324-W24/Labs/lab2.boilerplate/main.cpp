...
int main()
{
    ...
    queue<Process> slides;
    slides.push(Process(1, 14, 0));
    slides.push(Process(2, 11, 1));
    slides.push(Process(3, 15, 9));
    ...
    cout << "##### Round Robin Scheduling Algorithm: TEST CASE#2 #####" << endl;
    scheduler = new RoundRobinScheduler(slides, 4);
    // Run the scheduler
    scheduler->schedule();
